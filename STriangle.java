...
public class STriangle extends Shape {
	...
	public STriangle(STriangle _STriangle) {
		this.p1 = new Point(_STriangle.p1);
		this.p2 = new Point(_STriangle.p2);
		this.p3 = new Point(_STriangle.p3);
		this.loc = new Point(_STriangle.p1);
...
//		System.out.println(maxx-minx);
	...
	public void setBounds(int dx, int dy)
	{
		...
		if (this.p1.x==rect.width)
		{
			this.p1.x *= (rect.width+dx)/rect.width;
		}
		if (this.p2.x==rect.width)
		{
			this.p2.x *= (rect.width+dx)/rect.width;
		}
		if (this.p3.x==rect.width)
		{
			this.p3.x *= (rect.width+dx)/rect.width;
		}
		
		if (this.p1.y==rect.height)
		{
			this.p1.y *= (rect.height+dy)/rect.height;
		}
		if (this.p2.y==rect.height)
		{
			this.p2.y *= (rect.height+dy)/rect.height;
		}
		if (this.p3.y==rect.height)
		{
			this.p3.y *= (rect.height+dy)/rect.height;
		}
	}
}
